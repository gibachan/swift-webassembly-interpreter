...
final class WasmEncoderTests: XCTestCase {
    ...
    func testEncodeFuncAddFloat() throws {
        let fileName = "func_add_float"
        let decoder = try WasmDecoder(filePath: path(for: fileName))
        let wasm = try decoder.invoke()
    ...
    func testEncodeTableTest() throws {
        let fileName = "table_test"
        let decoder = try WasmDecoder(filePath: path(for: fileName))
        let wasm = try decoder.invoke()
    ...
    func testEncodeTableExport() throws {
    ...
    func testEncodeMemoryAccumulate() throws {
        let fileName = "memory_accumulate"
        let decoder = try WasmDecoder(filePath: path(for: fileName))
        let wasm = try decoder.invoke()
